...
st.set_page_config(page_title="Langchain with SQL DB",page_icon="🦜")
...
if radio_opt.index(selected_opt)==1:
    ...
    mysql_db=st.sidebar.text_input("provide My sql DB")
...
agent=create_sql_agent(llm,toolkit,verbose=True,handle_parsing_errors=True)
...
use_query=st.chat_input(placeholder="Ask anything from the database")

if use_query:
    st.session_state.messages.append({"role":"user","content":use_query})
    st.chat_message("user").write(use_query)

    with st.chat_message("assistant"):
        streamlit_callback=StreamlitCallbackHandler(st.container())
        response=agent.invoke({"input": use_query}, {"callbacks": [streamlit_callback]})
